...
func TestGetPathInfo(t *testing.T) {
	tempDir := testutil.TempDir(t, "pathinfo-test")

	// Create a regular file
	regularFile := filepath.Join(tempDir, "regular.txt")
	testutil.CreateTestFile(t, tempDir, "regular.txt", "test content")
	...
	// Create a symlink (skip on Windows if symlinks are not supported)
	...
	tests := []struct {
		...
		},
		{
			name:         "directory",
			path:         subDir,
			expectedType: PathTypeDir,
			wantErr:      false,
		},
		{
			name:         "symlink",
			path:         symlinkPath,
			expectedType: PathTypeSymlink,
			wantErr:      false,
		},
		{
			name:         "non-existent path",
			path:         filepath.Join(tempDir, "nonexistent"),
			expectedType: PathTypeOther,
			wantErr:      true,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			info, pathType, err := GetPathInfo(tt.path)

			if tt.wantErr {
				if err == nil {
					t.Errorf("GetPathInfo() expected error but got none")
				}
				return
			}
			...
			if pathType != tt.expectedType {
				t.Errorf("GetPathInfo() pathType = %v, want %v", pathType, tt.expectedType)
			}

			if info == nil {
				t.Error("GetPathInfo() returned nil fileInfo")
			}
		})
	}
}
...
func TestResolveSymlink(t *testing.T) {
	...
	tempDir := testutil.TempDir(t, "symlink-test")

	// Create a regular file
	regularFile := filepath.Join(tempDir, "target.txt")
	testutil.CreateTestFile(t, tempDir, "target.txt", "target content")
	...
	tests := []struct {
	...
	}{
		{
			...
			symlinkPath:  symlinkToFile,
			expectedType: PathTypeFile,
			wantErr:      false,
		},
		{
			name:         "symlink to directory",
			symlinkPath:  symlinkToDir,
			expectedType: PathTypeDir,
			wantErr:      false,
		},
		{
			name:        "broken symlink",
			symlinkPath: brokenSymlink,
			wantErr:     true,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			targetPath, targetInfo, targetType, err := ResolveSymlink(tt.symlinkPath)

			if tt.wantErr {
				if err == nil {
					t.Errorf("ResolveSymlink() expected error but got none")
				}
				return
			}
			...
			if targetType != tt.expectedType {
				t.Errorf("ResolveSymlink() targetType = %v, want %v", targetType, tt.expectedType)
			}

			if targetPath == "" {
				t.Error("ResolveSymlink() returned empty target path")
			}

			if targetInfo == nil {
				t.Error("ResolveSymlink() returned nil targetInfo")
			}
		})
	}
}
...
func TestShouldSkipHidden(t *testing.T) {
	tests := []struct {
		...
		{
			name:          "hidden file with includeHidden=false",
			filename:      ".hidden",
			includeHidden: false,
			expectedSkip:  true,
		},
		{
			name:          "hidden file with includeHidden=true",
			filename:      ".hidden",
			includeHidden: true,
			expectedSkip:  false,
		},
		{
			name:          "regular file with includeHidden=false",
			filename:      "regular.txt",
			includeHidden: false,
			expectedSkip:  false,
		},
		{
			name:          "regular file with includeHidden=true",
			filename:      "regular.txt",
			includeHidden: true,
			expectedSkip:  false,
		},
		{
			name:          ".gitignore with includeHidden=false",
			filename:      ".gitignore",
			includeHidden: false,
			expectedSkip:  true,
		},
		{
			name:          ".gitignore with includeHidden=true",
			filename:      ".gitignore",
			includeHidden: true,
			expectedSkip:  false,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := ShouldSkipHidden(tt.filename, tt.includeHidden)
			if result != tt.expectedSkip {
				t.Errorf("ShouldSkipHidden() = %v, want %v", result, tt.expectedSkip)
			}
		})
	}
}
