...
func TestRecoveryPurgesOldCommitted(t *testing.T) {
	...
	if res.Purged == 0 {
		t.Fatalf("expected purge of old committed transaction")
	}
	if _, err := os.Stat(filepath.Join(root, ".txn", txn.j.ID)); !os.IsNotExist(err) {
		t.Fatalf("txn dir should be removed")
	}
}
