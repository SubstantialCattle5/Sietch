...
func TestTransactionCommit(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(root, "data", "file.txt")); err != nil {
...
func TestRecoveryRollsBackOrCommitsPending(t *testing.T) {
	...
	if res.RolledBack == 0 && res.ResumedCommits == 0 {
		t.Fatalf("expected rollback or resume")
	}
	_, statErr := os.Stat(filepath.Join(root, "X.txt"))
	if res.ResumedCommits > 0 && statErr != nil {
		t.Fatalf("expected file promoted on resumed commit: %v", statErr)
	}
	if res.RolledBack > 0 && !os.IsNotExist(statErr) {
		t.Fatalf("expected file absent after rollback")
	}
}
...
func TestStageDeleteCommit(t *testing.T) {
	...
	if err := txn.StageDelete("victim.txt"); err != nil {
		t.Fatalf("stage delete: %v", err)
	}
	if _, err := os.Stat(targetPath); !os.IsNotExist(err) {
		t.Fatalf("file should be moved to trash before commit")
	}
	if err := txn.Commit(); err != nil {
		t.Fatalf("commit: %v", err)
	}
	if _, err := os.Stat(targetPath); !os.IsNotExist(err) {
		t.Fatalf("file should remain deleted after commit")
	}
}
...
func TestStageReplaceCommit(t *testing.T) {
	...
	txn, _ := Begin(root, nil)
	w, err := txn.StageReplace("file.txt")
	if err != nil {
		t.Fatalf("stage replace: %v", err)
	}
	w.Write([]byte("new"))
	w.Close()
	if err := txn.Commit(); err != nil {
	...
	if string(data) != "new" {
		t.Fatalf("expected replaced content, got %s", string(data))
	}
}
...
func TestStageReplaceRollback(t *testing.T) {
	...
	data, _ := os.ReadFile(path)
	if string(data) != "old" {
		t.Fatalf("expected original content, got %s", string(data))
	}
}
...
func TestDeleteNonexistent(t *testing.T) {
	root := t.TempDir()
	txn, _ := Begin(root, nil)
	if err := txn.StageDelete("missing.txt"); err != nil {
		t.Fatalf("stage delete missing: %v", err)
	}
	if err := txn.Commit(); err != nil {
		t.Fatalf("commit: %v", err)
	}
...
func TestChecksumRecorded(t *testing.T) {
	...
	w.Write([]byte("abc"))
	w.Close()
	if len(txn.j.Entries) != 1 {
		t.Fatalf("expected 1 entry, got %d", len(txn.j.Entries))
	}
	if txn.j.Entries[0].Checksum == "" {
		t.Fatalf("checksum not recorded")
	}
}
...
func TestCommitStateGuard(t *testing.T) {
	...
	if err := txn.Commit(); err == nil {
		t.Fatalf("expected error committing already committed txn")
	}
}
...
func TestRecoveryResumesCommit(t *testing.T) {
	...
	if res.ResumedCommits == 0 && res.RolledBack == 0 {
		t.Fatalf("expected resume or rollback action")
	}
}
