...
func Begin(vaultRoot string, metadata map[string]any) (*Transaction, error) {
    ...
    if err := os.MkdirAll(txnRoot, 0o755); err != nil { return nil, fmt.Errorf("create txn root: %w", err) }
    id := time.Now().UTC().Format("20060102T150405Z") + fmt.Sprintf("-%06d", time.Now().Nanosecond())
    dir := filepath.Join(txnRoot, id)
    if err := os.MkdirAll(dir, 0o755); err != nil { return nil, fmt.Errorf("create txn dir: %w", err) }
    ...
    if err := j.persist(); err != nil { return nil, err }
...
func (t *Transaction) StageCreate(finalRelPath string) (io.WriteCloser, error) {
    t.j.mu.Lock(); defer t.j.mu.Unlock()
    staged := filepath.Join(t.j.dir, "new", filepath.FromSlash(finalRelPath))
    if err := os.MkdirAll(filepath.Dir(staged), 0o755); err != nil { return nil, fmt.Errorf("stage create mkdir: %w", err) }
    f, err := os.Create(staged); if err != nil { return nil, fmt.Errorf("stage create open: %w", err) }
    h := sha256.New()
...
func (cw *createWriter) Close() error {
    if err := cw.f.Close(); err != nil { return err }
    fi, err := os.Stat(cw.staged); if err != nil { return err }
    sum := cw.hsh.Sum(nil)
    cw.t.j.mu.Lock(); defer cw.t.j.mu.Unlock()
...
func (t *Transaction) StageDelete(finalRelPath string) error {
    ...
    if err := os.MkdirAll(filepath.Dir(trash), 0o755); err != nil { return fmt.Errorf("stage delete mkdir: %w", err) }
    if err := os.Rename(abs, trash); err != nil { return fmt.Errorf("stage delete move: %w", err) }
...
func (t *Transaction) StageReplace(finalRelPath string) (io.WriteCloser, error) {
    ...
    trash := filepath.Join(t.j.dir, "trash", filepath.FromSlash(finalRelPath))
    if _, err := os.Stat(abs); err == nil {
        if err := os.MkdirAll(filepath.Dir(trash), 0o755); err != nil { return nil, fmt.Errorf("stage replace mkdir trash: %w", err) }
        if err := os.Rename(abs, trash); err != nil { return nil, fmt.Errorf("stage replace move: %w", err) }
    }
    staged := filepath.Join(t.j.dir, "new", filepath.FromSlash(finalRelPath))
    if err := os.MkdirAll(filepath.Dir(staged), 0o755); err != nil { return nil, fmt.Errorf("stage replace mkdir new: %w", err) }
    f, err := os.Create(staged); if err != nil { return nil, fmt.Errorf("stage replace open: %w", err) }
    h := sha256.New()
...
func (rw *replaceWriter) Close() error {
    if err := rw.f.Close(); err != nil { return err }
    fi, err := os.Stat(rw.staged); if err != nil { return err }
    sum := rw.hsh.Sum(nil)
    rw.t.j.mu.Lock(); defer rw.t.j.mu.Unlock()
...
func (t *Transaction) Commit() error {
    t.j.mu.Lock()
    if t.j.State != StatePending { t.j.mu.Unlock(); return fmt.Errorf("cannot commit in state %s", t.j.State) }
    t.j.State = StateCommitting
    if err := t.j.persistLocked(); err != nil { t.j.mu.Unlock(); return err }
    entries := append([]JournalEntry(nil), t.j.Entries...)
    t.j.mu.Unlock()
    for _, e := range entries {
        if e.Type == EntryCreate || e.Type == EntryReplace {
            if e.StagedPath == "" { return t.fail(fmt.Errorf("missing staged path for %s", e.FinalPath)) }
            finalAbs := filepath.Join(t.j.vaultRoot, filepath.FromSlash(e.FinalPath))
            if err := os.MkdirAll(filepath.Dir(finalAbs), 0o755); err != nil { return t.fail(fmt.Errorf("commit mkdir: %w", err)) }
            if err := os.Rename(e.StagedPath, finalAbs); err != nil { return t.fail(fmt.Errorf("commit promote %s: %w", e.FinalPath, err)) }
        }
    }
    for _, e := range entries {
        if (e.Type == EntryDelete || e.Type == EntryReplace) && e.OriginalBackupPath != "" { _ = os.Remove(e.OriginalBackupPath) }
    }
    t.j.mu.Lock(); t.j.State = StateCommitted; err := t.j.persistLocked(); t.j.mu.Unlock(); return err
}

func (t *Transaction) Rollback() error {
    t.j.mu.Lock()
    if t.j.State != StatePending && t.j.State != StateCommitting && t.j.State != StateFailed { t.j.mu.Unlock(); return nil }
    t.j.State = StateRollingBack
    if err := t.j.persistLocked(); err != nil { t.j.mu.Unlock(); return err }
    entries := append([]JournalEntry(nil), t.j.Entries...)
    t.j.mu.Unlock()
    for _, e := range entries { if (e.Type == EntryCreate || e.Type == EntryReplace) && e.StagedPath != "" { _ = os.Remove(e.StagedPath) } }
    for _, e := range entries {
        if (e.Type == EntryDelete || e.Type == EntryReplace) && e.OriginalBackupPath != "" {
            finalAbs := filepath.Join(t.j.vaultRoot, filepath.FromSlash(e.FinalPath))
            if _, err := os.Stat(finalAbs); err == nil { continue }
            _ = os.MkdirAll(filepath.Dir(finalAbs), 0o755)
            _ = os.Rename(e.OriginalBackupPath, finalAbs)
        }
    }
    t.j.mu.Lock(); t.j.State = StateRolledBack; err := t.j.persistLocked(); t.j.mu.Unlock(); return err
}

func (t *Transaction) fail(err error) error {
    t.j.mu.Lock(); t.j.State = StateFailed; _ = t.j.persistLocked(); t.j.mu.Unlock(); _ = t.Rollback(); return err
}

func (j *Journal) persist() error { j.mu.Lock(); defer j.mu.Unlock(); return j.persistLocked() }
func (j *Journal) persistLocked() error {
    data, err := json.MarshalIndent(j, "", "  "); if err != nil { return err }
